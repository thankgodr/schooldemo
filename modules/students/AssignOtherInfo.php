	...
	if(count($_REQUEST['values']) && count($_REQUEST['student']))
	{
		...
		if($values_count && $students_count)
			...
			DBQuery('UPDATE students SET '.substr($update,1).' WHERE STUDENT_ID IN ('.substr($students,1).')');
                        ...
                        if($values[1].trim()!='')
                                 DBQuery('UPDATE medical_info SET '.$fields[1].'=\''.$values[1].'\' WHERE STUDENT_ID IN ('.substr($students,1).')');
                        if($values[2].trim()!='')
                                 DBQuery('UPDATE medical_info SET '.$fields[2].'=\''.$values[2].'\' WHERE STUDENT_ID IN ('.substr($students,1).')');
                        if($values[3].trim()!='')
                                 DBQuery('UPDATE medical_info SET '.$fields[3].'=\''.$values[3].'\' WHERE STUDENT_ID IN ('.substr($students,1).')');
