 ...
 else
 {
   ...
   // for standard grade card
	...
	$extra['FROM'] .= ',student_report_card_grades sg1 LEFT OUTER JOIN report_card_grades rpg ON (rpg.ID=sg1.REPORT_CARD_GRADE_ID),
        ...
        $extra['WHERE'] .= ' AND rc_cp.MARKING_PERIOD_ID IN ('.$mp_list.')  AND cpv.COURSE_PERIOD_ID=rc_cp.COURSE_PERIOD_ID
					AND c.COURSE_ID = rc_cp.COURSE_ID AND sg1.STUDENT_ID=ssm.STUDENT_ID AND sp.PERIOD_ID=cpv.PERIOD_ID
...
if(!$_REQUEST['modfunc'])
{
	...
	if($_REQUEST['search_modfunc']=='list')
	{
		...
		$extra['extra_header_left'] .= '</TR>';
                $extra['extra_header_left'] .= '<TR>';
		$extra['extra_header_left'] .= '<TD><INPUT type=checkbox name=elements[incl_picture] value=Y>Include Pictute</TD></TR>';
