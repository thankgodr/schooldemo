...
if(clean_param($_REQUEST['modfunc'],PARAM_ALPHAMOD)=='update')
{
        if($_REQUEST['tab_id']!='new')
        {
            ...
            foreach($_REQUEST['values'] as $id=>$dt)
            {
              ...
              if($id=='new')
              {
                if($dt['UNWEIGHTED_GP']!='' && $dt['GPA_VALUE']=='')
                {
                    $_REQUEST['values'][$id]['GPA_VALUE']=$dt['UNWEIGHTED_GP'];
                }
                elseif($dt['UNWEIGHTED_GP']=='' && $dt['GPA_VALUE']!='')
                {
                    $_REQUEST['values'][$id]['UNWEIGHTED_GP']=$dt['GPA_VALUE'];
                }
              }
              else
              {
                  $get_gps=DBGet(DBQuery('SELECT GPA_VALUE as WEIGHTED_GP,UNWEIGHTED_GP FROM report_card_grades WHERE ID='.$id));
                  if(isset($dt['UNWEIGHTED_GP']) && !(isset($dt['GPA_VALUE'])) && $dt['UNWEIGHTED_GP']=='' )
                  {
                       $_REQUEST['values'][$id]['UNWEIGHTED_GP']=$get_gps[1]['WEIGHTED_GP'];
                  }
                  elseif(isset($dt['UNWEIGHTED_GP']) && isset($dt['GPA_VALUE']) && $dt['UNWEIGHTED_GP']=='')
                  {
                       $_REQUEST['values'][$id]['UNWEIGHTED_GP']=$dt['GPA_VALUE'];
                  }
                  elseif(isset($dt['GPA_VALUE']) && !(isset($dt['UNWEIGHTED_GP'])) && $dt['GPA_VALUE']=='')
                  {
                       $_REQUEST['values'][$id]['GPA_VALUE']=$get_gps[1]['UNWEIGHTED_GP'];
                  }
                  elseif(isset($dt['GPA_VALUE']) && isset($dt['UNWEIGHTED_GP']) && $dt['GPA_VALUE']=='')
                  {
                       $_REQUEST['values'][$id]['GPA_VALUE']=$dt['UNWEIGHTED_GP'];
                                ...
                                if(!(isset($columns['TITLE']) && trim($columns['TITLE'])==''))
                               {
                                    ...
                                    $flag=1;
                                     ...
                                     ##############end#####################################    
                                            ...
                                            if($_REQUEST['tab_id']!='new')
                                            {
                                                $validate_title=DBGet(DBQuery('SELECT COUNT(1) as TITLE_EX FROM report_card_grades WHERE SYEAR='.UserSyear().' AND SCHOOL_ID='.UserSchool().' AND UPPER(TITLE)=\''.strtoupper($title).'\' AND ID!=\''.$id.'\''));
