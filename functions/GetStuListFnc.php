	...
	switch(User('PROFILE'))
	{
		case 'admin':
			...
			else
			{
				...
				$sql .= ','.db_case(array('(ssm.SYEAR=\''.UserSyear().'\' AND (ssm.START_DATE IS NOT NULL AND \''.date('Y-m-d',strtotime($extra['DATE'])).'\'>=ssm.START_DATE AND ((\''.date('Y-m-d',strtotime($extra['DATE'])).'\'<=ssm.END_DATE OR ssm.END_DATE IS NULL)) OR ssm.DROP_CODE='.$get_rollover_id.') )','true',"'<FONT color=green>Active</FONT>'","'<FONT color=red>Inactive</FONT>'")).' AS ACTIVE ';
...
{	global $_openSIS;
	...
	if($_REQUEST['addr'])
	{
		$sql .= ' AND (LOWER(sam.STREET_ADDRESS_1) LIKE \'%'.str_replace("'","\'",strtolower(trim($_REQUEST['addr']))).'%\' OR LOWER(sam.CITY) LIKE \''.str_replace("'","\'",strtolower(trim($_REQUEST['addr']))).'%\' OR LOWER(sam.STATE)=\''.str_replace("'","\'",strtolower(trim($_REQUEST['addr']))).'\' OR ZIPCODE LIKE \''.trim(str_replace("'","\'",$_REQUEST['addr'])).'%\')';
		if(!$extra['NoSearchTerms'])
...
{	global $_openSIS;
	...
	if($_REQUEST['addr'])
	{
		$sql .= ' AND (LOWER(a.STREET_ADDRESS_1) LIKE \'%'.str_replace("'","\'",strtolower(trim($_REQUEST['addr']))).'%\' OR LOWER(a.CITY) LIKE \''.str_replace("'","\'",strtolower(trim($_REQUEST['addr']))).'%\' OR LOWER(a.STATE)=\''.str_replace("'","\'",strtolower(trim($_REQUEST['addr']))).'\' OR ZIPCODE LIKE \''.trim(str_replace("'","\'",$_REQUEST['addr'])).'%\')';
                $_SESSION['newsql1'].= ' AND (LOWER(a.STREET_ADDRESS_1) LIKE \'%'.str_replace("'","\'",strtolower(trim($_REQUEST['addr']))).'%\' OR LOWER(a.CITY) LIKE \''.str_replace("'","\'",strtolower(trim($_REQUEST['addr']))).'%\' OR LOWER(a.STATE)=\''.str_replace("'","\'",strtolower(trim($_REQUEST['addr']))).'\' OR ZIPCODE LIKE \''.trim(str_replace("'","\'",$_REQUEST['addr'])).'%\')';
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>Address contains: </b></font>'.trim($_REQUEST['addr']).'<BR>';
	}
	if($_REQUEST['preferred_hospital'])
