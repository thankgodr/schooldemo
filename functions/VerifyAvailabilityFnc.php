...
function VerifyVariableSchedule_Update($columns)
{
    ...
    {
      $sql_op="SELECT cp.COURSE_PERIOD_ID FROM course_periods  cp LEFT JOIN course_period_var cpv ON (cp.course_period_id=cpv.course_period_id) WHERE cpv.PERIOD_ID='".$per_id."' AND cp.COURSE_PERIOD_ID!='".$columns['COURSE_PERIOD_ID']."' AND (secondary_teacher_id IN ($all_teacher) OR teacher_id IN ($all_teacher)){$mp_append_sql}{$days_append_sql}";
